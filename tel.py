    ...
    while parts:
        ...
        if matches:
            ...
            for match in matches:
                ...
                if download_link:
                    ...
                    if response.status_code == 200:  # Link is valid
                        results.append(f"✅ [{match['title']} by {match['author']}]({download_link})")
                    elif response.status_code == 403:  # Link is forbidden
                        read_online_link = f"https://archive.org/details/{match['identifier']}"
                        results.append(f"⚠️ [{match['title']} by {match['author']}] can't be downloaded, but can be read here: [Read Online]({read_online_link})")
                else:
            ...
            if results:
                ...
                await update.message.reply_text("❌ *No results found for your search.*", parse_mode="Markdown")
